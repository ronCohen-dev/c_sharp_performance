...
public class PerformanceDeepComparisonMongo
{
    private readonly MongoContext _mongo;

    public PerformanceDeepComparisonMongo()
    {
        _mongo = new MongoContext();
    }

    public async Task RunAsync()
    {
        var mongoData = await _mongo.Persons.Find(_ => true).ToListAsync();
        Console.WriteLine($" {mongoData.Count:N0} rows in mongo \n");

        var results = new List<(string Name, long Time, int Count, string Explanation)>
        {
            Measure("For Loop", mongoData, FilterWithFor, "low-level iteration\""),
            Measure("Foreach", mongoData, FilterWithForeach, "enumerator overhead"),
            Measure("LINQ", mongoData, FilterWithLinq, "iterator layers"),
            Measure("Parallel LINQ", mongoData, FilterWithParallelLinq, "multi-core execution"),
            Measure("Span<T>", mongoData, FilterWithSpan, "memory efficient"),
            Measure("ValueLinq", mongoData, FilterWithValueLinq, "fusion optimization")
        };
